...
public:
    ...
    virtual std::unique_ptr<Analytical> make_unique() const = 0;

    double numerical_derivative(double x, int n, double eps) const;
};

class pow : public Analytical {
private:
    double expo;
public:
    pow(double expo) : expo{expo}{}
    double derivative(double x, int n) const override;
    std::unique_ptr<Analytical> make_unique() const override;
};

class log : public Analytical {
public:
    double derivative(double x, int n) const override;
    std::unique_ptr<Analytical> make_unique() const override;
};

class exp : public Analytical {
public:
    double derivative(double x, int n) const override;
    std::unique_ptr<Analytical> make_unique() const override;
};

class sin : public Analytical {
public:
    double derivative(double x, int n) const override;
    std::unique_ptr<Analytical> make_unique() const override;
};

class cos : public Analytical {
public:
    double derivative(double x, int n) const override;
    std::unique_ptr<Analytical> make_unique() const override;
};

class Constant : public Analytical {
private:
    double value;
public:
    Constant(double value) : value{value} {}
    Constant(int value) : value{static_cast<double>(value)}{}
    double derivative(double x, int n) const override;
    std::unique_ptr<Analytical> make_unique() const override;
};

class Composed : public Analytical {
private:
    std::unique_ptr<Analytical> inner, outer;
public:
    Composed(const Analytical& outer, const Analytical& inner);
...
class Product : public Analytical {
private:
    std::unique_ptr<Analytical> f, g;
public:
    Product() = delete;
    Product(const Analytical& f, const Analytical& g);
...
class Sum : public Analytical {
private:
    std::vector<std::unique_ptr<Analytical>> terms;
public:

    Sum() = default;
    Sum(const Analytical& t1, const Analytical& t2);
    Sum(const Sum& other);
...
#define ANALYTICAL_MIXED_ARITHMETIC(OP)                                                   \
  template<typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>         \
  auto operator OP (T lhs, const Analytical& rhs) {                                       \
    return operator OP(Constant(static_cast<double>(lhs)), rhs);                          \
  }                                                                                       \
  template<typename T, typename = std::enable_if_t<std::is_arithmetic<T>::value>>         \
  auto operator OP (const Analytical& lhs, T rhs) {                                       \
    return operator OP(lhs, Constant(static_cast<double>(rhs)));                          \
  }

ANALYTICAL_MIXED_ARITHMETIC(+)
ANALYTICAL_MIXED_ARITHMETIC(-)
ANALYTICAL_MIXED_ARITHMETIC(*)
ANALYTICAL_MIXED_ARITHMETIC(/)
#undef ANALYTICAL_MIXED_ARITHMETIC

class PolyExp;
class Polynomial : public Analytical {
public:
    ...
    */
    ...
    virtual std::unique_ptr<Analytical> make_unique() const override;
// private:
...
class PolyExp : public Analytical{
public:
    /*
...
class PolyFrac : public Analytical {
public:
    PolyFrac(Polynomial num, Polynomial denom);
    double operator()(double x) const override;
    double derivative(double x, int n) const override;
    double get_Hk(double x, int k, const vector1d& dg) const;

    friend std::ostream& operator<<(std::ostream& strm, const PolyFrac& p);
    virtual std::unique_ptr<Analytical> make_unique() const override;
private:
